...
# Convertir a minúsculas para comparaciones insensibles
PALABRAS_IRRELEVANTES_LOWER = set(palabra.lower() for palabra in PALABRAS_IRRELEVANTES)
...
def extraer_lugares(texto):
    ...
    for lugar in lugares_regex:
        if lugar.lower() in PALABRAS_IRRELEVANTES_LOWER or len(lugar) < 4:  # Ignorar irrelevantes y cortos
        ...
        if pais_local and estado_local:
            ...
            # Si se encuentra al menos un lugar completo, detener búsqueda adicional
            print("Lugar completo validado. Deteniendo búsqueda adicional.")
            return pais, estado, municipio, None, justificacion  # Detiene todo el procesamiento adicional
