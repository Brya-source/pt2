...
def agregar_campos():
    try:
        ...
        cursor.execute("SHOW COLUMNS FROM extracciones LIKE 'pais2'")
        resultado_pais = cursor.fetchone()

        cursor.execute("SHOW COLUMNS FROM extracciones LIKE 'estado2'")
        resultado_estado = cursor.fetchone()

        cursor.execute("SHOW COLUMNS FROM extracciones LIKE 'municipio2'")
        resultado_municipio = cursor.fetchone()

        cursor.execute("SHOW COLUMNS FROM extracciones LIKE 'ciudad2'")
        resultado_ciudad = cursor.fetchone()

        # Agregar los campos si no existen
        if not resultado_pais:
            cursor.execute("ALTER TABLE extracciones ADD COLUMN pais2 VARCHAR(255)")
            print("Campo 'pais' agregado.")

        if not resultado_estado:
            cursor.execute("ALTER TABLE extracciones ADD COLUMN estado2 VARCHAR(255)")
            print("Campo 'estado' agregado.")

        if not resultado_municipio:
            cursor.execute("ALTER TABLE extracciones ADD COLUMN municipio2 VARCHAR(255)")
            print("Campo 'municipio' agregado.")

        if not resultado_ciudad:
            cursor.execute("ALTER TABLE extracciones ADD COLUMN ciudad2 VARCHAR(255)")
...
def actualizar_base_datos(pais, estado, municipio, ciudad, tipo_localidad, noticia_id):
    try:
        ...
        sql = "UPDATE extracciones SET pais2=%s, estado2=%s, municipio2=%s, ciudad2=%s WHERE id=%s"
...
def procesar_noticias():
    try:
        ...
        sql = "SELECT id, noticia_corregida, pais2, estado2, municipio2, ciudad2 FROM extracciones WHERE relacion_bart3='Sí'"
        cursor.execute(sql)
